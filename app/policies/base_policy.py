...
class PolicyContext:
    ...
    def get_membership(self) -> Optional["Membership"]:
        ...
        for membership in self.user.memberships:
            if membership.organization_id == self.organization_id and membership.is_active:
                return membership
        return None
...
class BasePolicy(ABC):
    ...
    def _require_authentication(self, context: PolicyContext) -> PolicyResult | None:
    ...
    def _require_organization_membership(self, context: PolicyContext) -> PolicyResult | None:
        """Check if user is a member of the organization."""
    ...
    def _require_role(self, context: PolicyContext, required_role: Role) -> PolicyResult | None:
