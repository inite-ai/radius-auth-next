...
class OAuthIdentity(Base, TimestampMixin):
    ...
    provider_user_id: Mapped[str] = mapped_column(String(255), nullable=False, index=True)  # OAuth provider's user ID
    ...
    access_token: Mapped[Optional[str]] = mapped_column(Text)  # Encrypted in production
    refresh_token: Mapped[Optional[str]] = mapped_column(Text)  # Encrypted in production
    token_expires_at: Mapped[Optional[str]] = mapped_column(String(255))
    
    # Provider-specific data (JSON)
    provider_data: Mapped[Optional[str]] = mapped_column(Text)  # JSON with additional provider data
