...
class TestAPIKeySecurity:
    ...
    async def test_api_key_usage_tracking(
        self, async_client: AsyncClient, create_api_key, db_session
    ):
        ...
        result = await db_session.execute(select(APIKey).where(APIKey.id == create_api_key.id))
    ...
    async def test_api_key_rate_limiting(
        self, async_client: AsyncClient, create_api_key_with_low_rate_limit, mock_redis
    ):
        ...
        ), f"First request should be 200, got {responses[0].status_code}"
        ...
        if not rate_limited:
            ...
            ), "All requests should succeed if rate limiting not working"
    ...
    async def test_api_key_hash_storage(self, async_client: AsyncClient, auth_headers, db_session):
        ...
        result = await db_session.execute(select(APIKey).where(APIKey.name == "Hash Test Key"))
        api_key_record = result.scalar_one()
...
class TestAPIKeyManagement:
    ...
    @pytest.mark.asyncio
    async def test_revoke_other_users_api_key(
        self, async_client: AsyncClient, create_api_key, create_admin_user, admin_auth_headers
    ):
